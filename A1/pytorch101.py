...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    arr = [i for i in range(start, stop+1) if i % 10 == 0]
    x = torch.tensor(arr, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x[:2, :6] = 2
    x[0:2, 0] = torch.tensor([0, 0])
    x[0:2, 1] = torch.tensor([1, 1])
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    idx_col = [i for i in range(n)]
    for i in range(m):
        y[[i] * n, idx_col] = x[[m - i - 1] * n, idx_col]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
